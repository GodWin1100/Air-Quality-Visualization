...
const MapChart = ({ setTooltipContent }) => {
  ...
  useEffect(()=>{
    getData()
  },[data]) // useEffect renders each time whatever's inside square bracket changes. If empty, it only changes onMount.
        ...
        >
          <Geographies geography={geoUrl}>
            {({ geographies }) => (
              <>
              {geographies.map((geo) => {
                const cur = data.find((data) => data.Ward === geo.properties.name);
                return (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
